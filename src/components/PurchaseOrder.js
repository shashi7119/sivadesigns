...
const PurchaseOrder = () => {
  ...
  const handleAddItem = () => {
    setItems([...items, { id: Date.now(), itemId: '', quantity: 1,unit: "",tax: 0, price: 0 }]);
  };
  ...
  const handleItemSelectChange = (id, event) => {
    ...
    const selectedItem = availableItems.find((item) => item[1] === selectedItemId);
   
    const updatedItems = items.map((item) =>
      item.id === id
        ? {
            ...item,
            itemId: selectedItem ? selectedItem[1] : '',
  ...
  const handlePriceChange = (id, event) => {
   ...
   const handleTaxChange = (id, event) => {
    const { value } = event.target;
    const updatedItems = items.map((item) =>
      item.id === id ? { ...item, tax: parseFloat(value) || 0 } : item
    );
    setItems(updatedItems);
  };
  ...
  const handleSubmit = (event) => {
    ...
    const purchaseOrderData = {
      ...
      items: items.map((item) => ({ itemId: item.itemId, quantity: item.quantity, tax: item.tax,price: item.price })),
  ...
  const handleNewOrder = () => {
    ...
    setSelectedVendorId('');
    setItems([{ id: Date.now(), itemId: '', name: '', quantity: 1, unit: '', tax: 5, price: 0 }]);
  };
                      ...
                      <tr key={item.id}>
                        <td className="px-6 py-4">
                          ...
                          >
                            ...
                            {availableItems.map((availableItem) => (
                              <option key={availableItem[1]} value={availableItem[1]}>
                                {availableItem[1]}
                              </option>
                            ))}
                          </Form.Select>
                        ...
                        <td className="px-6 py-4">
                          <Form.Control
                            type="number"
                            value={item.tax}
                            onChange={(event) => handleTaxChange(item.id, event)}
                            min="0"
                            step="0.01"
                            className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                          />
